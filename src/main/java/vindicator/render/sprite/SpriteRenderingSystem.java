...
public class SpriteRenderingSystem extends EntitySystem {
    ...
    private void drawScreenText(SpriteBatch batch) {
        ...
        for (Entity entity : screenTextsImmutableArray) {
            final Transform transform = entity.getComponent(HudTransformComponent.class).getTransform();
            bitmapFontProvider.get().draw(batch, entity.getComponent(TextComponent.class).getText(),transform.getPosition().x, transform.getPosition().y);
        }
    }
